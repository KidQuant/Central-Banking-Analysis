...
class FomcTestimony(FomcBase):
    ...
    def _get_links(self, from_year):
        ...
        if from_year < 1996:
            ...
            print(
                "All data from 2006 is in a single json, so return all from 2006 anyway though specified from year is ",
                from_year,
            )
        ...
        if from_year < 2006:
            for year in range(from_year, 2006):
                ...
                doc_links = soup.findAll(
                    "a",
                    href=re.compile(
                        "^/boarddocs/testimony/{}/|^/boarddocs/hh/{}/".format(
                            str(year), str(year)
                        )
                    ),
                )
                for doc_link in doc_links:
                    ...
                    if doc_link.get("href") in (
                        "/boarddocs/testimony/2005/20050420/default.htm"
                    ):
                        title = doc_link.get_text()
                        speaker = (
                            doc_link.parent.parent.next_element.next_element.get_text()
                            .replace("\n", "")
                            .strip()
                        )
                    ...
                    elif doc_link.get("href") in (
                        "/boarddocs/testimony/1997/19970121.htm"
                    ):
                        title = (
                            doc_link.parent.parent.find_next("em")
                            .get_text()
                            .replace("\n", "")
                            .strip()
                        )
                        speaker = (
                            doc_link.parent.parent.find_next("strong")
                            .get_text()
                            .replace("\n", "")
                            .strip()
                        )
                ...
                if self.verbose:
                    print(
                        "YEAR: {} - {} testimony docs found.".format(
                            year, len(doc_links)
                        )
                    )
