...
class FomcSpeech(FomcBase):
    ...
    def _get_links(self, from_year):
        ...
        for year in range(from_year, 2006):
            ...
            soup = BeautifulSoup(res.text, "html.parser")
            speech_links = soup.findAll(
                "a",
                href=re.compile(
                    "^/?newsevents/speech/.*{}\d\d\d\d.*.htm|^/boarddocs/speeches/{}/|^{}\d\d\d\d.*.htm".format(
            ...
            for speech_link in speech_links:
                ...
                self.dates.append(
                    datetime.strptime(
                        self._date_from_link(speech_link.attrs["href"]), "%Y-%m-%d"
                    )
                )
                ...
                if year == 1997:
                    ...
                    if (
                        speech_link.get("href")
                        == "/boarddocs/speeches/1997/19971215.htm"
                    ):
                        tmp_speaker = (
                            speech_link.parent.next_sibling.next_element.get_text()
                            .replace("\n", "")
                            .strip()
                        )
                    else:
                        tmp_speaker = (
                            speech_link.parent.previous_sibling.previous_sibling.get_text()
                            .replace("\n", "")
                            .strip()
                        )
                else:
                    # Somehow 20051128 and 20051129 are structured differently
                    if speech_link.get("href") in (
                        "/boarddocs/speeches/2005/20051128/default.htm",
                    ...
                    if tmp_speaker in ("Watch Live", "Video"):
                        tmp_speaker = (
                            speech_link.parent.next_sibling.next_sibling.next_sibling.next_element.get_text()
                            .replace("\n", "")
                            .strip()
                        )
                self.speakers.append(tmp_speaker)
            if self.verbose:
                print("YEAR: {} - {} speeches found.".format(year, len(speech_links)))
